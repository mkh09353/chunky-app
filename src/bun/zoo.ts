...
export function createZooManager(deps: ZooDependencies = {}) {
  ...
  const database = () => {
    ...
    db.run("CREATE TABLE IF NOT EXISTS ideas (id TEXT PRIMARY KEY, pass_id TEXT NOT NULL REFERENCES passes(id), type TEXT NOT NULL, title TEXT NOT NULL, rationale TEXT NOT NULL, status TEXT NOT NULL, created_at INTEGER NOT NULL, item_id TEXT)")
  ...
  const ideaFrom = (row: Row): ZooIdea => ({ id: String(row.id), type: row.type as ZooIdeaType, title: String(row.title), rationale: String(row.rationale), status: row.status as ZooIdeaStatus, insightIds: insightIds(String(row.id)), createdAt: Number(row.created_at), ...(typeof row.item_id === "string" && row.item_id ? { itemId: row.item_id } : {}) })
